	...
	<view :style='{"minHeight":"100%","width":"100%","padding":"0","position":"relative","background":"#fff","height":"auto"}'>
		<form :style='{"width":"100%","padding":"24rpx","background":"none","display":"block","height":"auto"}' class="app-update-pv">
			<view :style='{"padding":"12rpx 0","margin":"0 0 24rpx 0","borderColor":"#d0d0d0","alignItems":"center","borderWidth":"0 0 2rpx 0","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' class="">
				...
				<input :style='{"border":"none","padding":"0px 24rpx","margin":"0px","color":"rgb(0, 0, 0)","borderRadius":"40rpx","flex":"1","background":"rgba(255, 255, 255, 0)","fontSize":"28rpx","height":"80rpx"}' :disabled="ro.jiankangfenlei" v-model="ruleForm.jiankangfenlei" placeholder="健康分类"  type="text"></input>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`jiankangfenlei`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
				...
				//跨表计算判断
				var obj;
				if((!this.ruleForm.jiankangfenlei)){
					this.$utils.msg(`健康分类不能为空`);
					return
				}
				//更新跨表属性
				var crossuserid;
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`jiankangfenlei`, params);
				...
				//跨表计算
						let oet = {}
						if(this.ruleForm.id){
							await this.$api.update(`jiankangfenlei`, this.ruleForm);
						}else{
							oet = await this.$api.add(`jiankangfenlei`, this.ruleForm);
						}
						if(this.cross){
