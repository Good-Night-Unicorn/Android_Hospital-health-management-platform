	...
	<div>
		...
		<div class="register-container">
			<el-form v-if="pageFlag=='register'" ref="ruleForm" class="rgs-form animate__animated animate__" :model="ruleForm" :rules="rules">
				<div class="rgs-form2">
					<div class="title">基于Android的医院健康管理平台的设计与实现</div>
					...
					<el-form-item class="list-item" v-if="tableName=='huanzhe'">
						<div class="lable" :class="changeRules('jiashudianhua')?'required':''">家属电话：</div>
						<el-input  v-model="ruleForm.jiashudianhua"  autocomplete="off" placeholder="家属电话"  type="text"  />
					</el-form-item>
...
export default {
	...
	mounted(){
		...
		if(this.$route.query.pageFlag=='register'){
			...
			if ('huanzhe' == this.tableName) {
				this.rules.jiashudianhua = [{ required: true, message: '请输入家属电话', trigger: 'blur' }]
			}
	...
	methods: {
		...
		login() {
			...
			if((!this.ruleForm.zhuzhi) && `huanzhe` == this.tableName){
				this.$message.error(`住址不能为空`);
				return
			}
			if((!this.ruleForm.jiashudianhua) && `huanzhe` == this.tableName){
				this.$message.error(`家属电话不能为空`);
				return
			}
			if(`huanzhe` == this.tableName && this.ruleForm.jiashudianhua &&(!this.$validate.isMobile(this.ruleForm.jiashudianhua))){
...
.register-container {
	position: relative;
	background: url(http://codegen.caihongy.cn/20240926/67d8b8cb5e9e4bce9d9e8a301757a3ed.jpg) no-repeat center center / 100% 100% !important;
	background: url(http://codegen.caihongy.cn/20240926/67d8b8cb5e9e4bce9d9e8a301757a3ed.jpg) no-repeat center center / 100% 100% !important;
	display: flex;
	width: 100%;
	min-height: 100vh;
