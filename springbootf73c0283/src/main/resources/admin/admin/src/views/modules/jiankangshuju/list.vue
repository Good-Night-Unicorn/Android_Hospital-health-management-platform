<template>
	<div class="main-content" :style='{"width":"100%","padding":"20px 40px","fontSize":"15px","overflow":"hidden"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"0px solid #fff","margin":"0 0px 20px","flexWrap":"wrap","background":"none","display":"flex","width":"100%","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"padding":"0px","margin":"0px 0","flexWrap":"wrap","background":"none","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('jiankangshuju','血糖统计')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#939393","height":"34px"}'></span>
						血糖统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('jiankangshuju','血压统计')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#939393","height":"34px"}'></span>
						血压统计
					</el-button>
				</el-row>
			...
			<div :style='{"border":"0px solid #fff","padding":"0","color":"#000","borderRadius":"10px","background":"#fff","borderWidth":"0px","width":"100%","fontSize":"14px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column  :resizable='true' prop="baogaotupian" width="200" label="报告图片">
						<template slot-scope="scope">
							<div v-if="scope.row.baogaotupian">
								<img v-if="scope.row.baogaotupian.substring(0,4)=='http'&&scope.row.baogaotupian.split(',w').length>1" :src="scope.row.baogaotupian" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.baogaotupian)">
								<img v-else-if="scope.row.baogaotupian.substring(0,4)=='http'" :src="scope.row.baogaotupian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.baogaotupian.split(',')[0])">
								<img v-else :src="$base.url+scope.row.baogaotupian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView($base.url+scope.row.baogaotupian.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
		...
		<el-dialog
			...
			width="800">
			<div id="xuetangChart1" style="width:100%;height:600px;"></div>
		...
		<el-dialog
			...
			width="800">
			<div id="shousuoyaChart2" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
	...
	export default {
		...
		methods: {
			...
			chartDialog1() {
				...
				this.$nextTick(()=>{
					var xuetangChart1 = echarts.init(document.getElementById("xuetangChart1"),'blue');
					this.$http({
						url: "jiankangshuju/sectionStat/xuetang",
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							// 使用刚指定的配置项和数据显示图表。
							xuetangChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								xuetangChart1.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			chartDialog2() {
				...
				this.$nextTick(()=>{
					...
					var shousuoyaChart2 = echarts.init(document.getElementById("shousuoyaChart2"),'blue');
					this.$http({
						url: "jiankangshuju/sectionStat/shousuoya",
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							for(let i=0;i<res.length;i++){
								...
								xAxis.push(res[i].shousuoya);
								yAxis.push(parseFloat((res[i].total)));
								pArray.push({
									value: parseFloat((res[i].total)),
									name: res[i].shousuoya
								})
							}
							var option = {};
							let titleObj = this.bar.title
							...
							// 使用刚指定的配置项和数据显示图表。
							shousuoyaChart2.setOption(option);
							//根据窗口的大小变动图表
							window.onresize = function() {
								shousuoyaChart2.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			init () {
				...
				urlxuetang+=`?remindend=4`,
				
				urlxuetang+=`&remindstart=11`,
				...
				this.$http({
					...
					if (data && data.code == 0 && data.count>0 ) {
						...
						this.$msgbox({
							message: h('p', null, [
								h('div', { style: 'text-align: center;font-weight: bold;font-size: 20px;padding: 20px 0' }, `血糖(mmol/L)预警`),
								h('p', null, [
				...
				let urlshousuoya = 'jiankangshuju/remind/shousuoya/1';
				urlshousuoya+=`?remindend=90`,
				
				urlshousuoya+=`&remindstart=140`,
				...
				this.$http({
					...
					if (data && data.code == 0 && data.count>0 ) {
						...
						this.$msgbox({
							message: h('p', null, [
								h('div', { style: 'text-align: center;font-weight: bold;font-size: 20px;padding: 20px 0' }, `收缩压(kpa)预警`),
								h('p', null, [
									h('div', { style: 'text-align: center;font-weight: bold;padding: 0 0 10px;font-size: 16px' }, `${data.count}条记录到达预警`),
									h('div', { style: 'text-align: center' }, [
										h('span', null, `包含：`),
