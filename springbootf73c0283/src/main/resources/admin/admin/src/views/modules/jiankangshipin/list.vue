		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"0px solid #fff","margin":"0 0px 20px","flexWrap":"wrap","background":"none","display":"flex","width":"100%","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex"}' >
					...
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}' class="select" label="健康分类" prop="jiankangfenlei">
						...
						<el-select clearable v-model="searchForm.jiankangfenlei" placeholder="请选择健康分类" >
							<el-option v-for="(item,index) in jiankangfenleiOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			init () {
				this.$http({
					url: `option/jiankangfenlei/jiankangfenlei`,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
						this.jiankangfenleiOptions = data.data;
					} else {
