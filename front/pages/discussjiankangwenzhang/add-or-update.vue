	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussjiankangwenzhang`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussjiankangwenzhang`, params);
				...
				//跨表计算
						let oet = {}
						if(this.ruleForm.id){
							await this.$api.update(`discussjiankangwenzhang`, this.ruleForm);
						}else{
							oet = await this.$api.add(`discussjiankangwenzhang`, this.ruleForm);
						}
						uni.setStorageSync('discussjiankangwenzhangCleanType',true);
						if(this.cross){
