			...
			<view :style='{"minHeight":"100%","padding":"0 20rpx","flexWrap":"wrap","background":"#E9E9E9","display":"flex","width":"100%","position":"relative","height":"auto"}'>
				<view class="cu-bar bg-white search" :style='{"padding":"20rpx 0 0 0","margin":"0","background":"none","display":"flex","width":"100%","position":"relative","height":"auto","order":"1"}'>
					<view :style='{"border":"2rpx solid #3d4e81","margin":"0","borderRadius":"20rpx","flex":"1","background":"#fff","lineHeight":"80rpx","position":"relative","height":"80rpx"}' class="search-form round">
						...
						<input placeholder-style="color: #333" :style='{"border":"0","padding":"12rpx 20rpx 12rpx 80rpx","color":"#333","borderRadius":"40rpx","background":"none","width":"100%","lineHeight":"80rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.biaoti" type="text" placeholder="标题" ></input>
					</view>
				...
				<view :style='{"width":"100%","flexWrap":"wrap","background":"none","display":"flex","height":"auto","order":"5"}'>
					<scroll-view scroll-x="true" class="category-one" :style='{"padding":"0","margin":"20rpx 0","whiteSpace":"nowrap","borderRadius":"20rpx","background":"none","flexDirection":"row","display":"flex","width":"100%","height":"auto"}'>
						<view :class='categoryName === item.jiankangfenlei ? "active" : ""' class="tab" v-for="(item,index) in categoryList" :key="index" @tap="categoryClick(item.jiankangfenlei)">{{item.jiankangfenlei}}</view>
					</scroll-view>
					<!-- 样式2 -->
					<view class="list" :style='{"width":"100%","padding":"0","margin":"0","borderRadius":"20rpx","background":"none","height":"auto"}'>
						<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"boxShadow":"0 2rpx 12rpx rgba(0,0,0,.3)","padding":"20rpx","margin":"0 0 20rpx","borderRadius":"20rpx","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","height":"auto"}' v-for="(product,index) in list" :key="index">
							...
							<view class="listmr" :style='{"width":"calc(100% - 200rpx)","padding":"0","margin":"0","height":"auto"}'>
								<view class="col3 f30 elip mb15" :style='{"padding":"0 20rpx","margin":"0","overflow":"hidden","color":"#333","width":"100%","lineHeight":"48rpx","fontSize":"28rpx"}'>{{product.biaoti}}</view>
								<view class="col3 f30 elip mb15" :style='{"padding":"0 20rpx","margin":"0","overflow":"hidden","color":"#333","width":"100%","lineHeight":"48rpx","fontSize":"28rpx"}'>{{product.jiankangfenlei}}</view>
							...
							<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('jiankangwenzhang','修改')) || (!userid && isAuthFront('jiankangwenzhang','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
								...
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('jiankangwenzhang','删除')) || (!userid && isAuthFront('jiankangwenzhang','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
							...
							<!-- #ifndef MP-WEIXIN -->
							<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('jiankangwenzhang','修改')) || (!userid && isAuthFront('jiankangwenzhang','修改'))" @tap.stop.proevent="onUpdateTap(product)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}' class="cuIcon-edit"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}'>修改</text>
								</view>
								<view :style='{"display":"flex"}' v-if="(userid && isAuth('jiankangwenzhang','删除')) || (!userid && isAuthFront('jiankangwenzhang','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
									<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}' class="cuIcon-delete"></text>
									<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}'>删除</text>
								</view>
							</view>
...
<script>
	export default {
		...
		async onShow() {
            ...
            let res = {};
            if(this.userid) {
                res = await this.$api.page('jiankangfenlei', {page:1,limit:100});
            } else {
                res = await this.$api.list('jiankangfenlei', {page:1,limit:100});
            }
			res.data.list.splice(0,0,{id:0,jiankangfenlei:'全部'})
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if(this.categoryName!='全部'){
					params.jiankangfenlei = '%' + this.categoryName + '%'
				}
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`jiankangwenzhang`, params);
                } else {
                    res = await this.$api.list(`jiankangwenzhang`, params);
                }
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('jiankangwenzhang', JSON.stringify([id]));
			...
			async search(){
				...
				if(this.categoryName!='全部'){
					searchForm.jiankangfenlei = '%' + this.categoryName + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`jiankangwenzhang`, searchForm);
                } else {
                    res = await this.$api.list(`jiankangwenzhang`, searchForm);
                }
				// 如果是第一页数据置空
