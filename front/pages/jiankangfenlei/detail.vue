	...
	export default {
		...
		async onShow(options) {
			...
			if(crossCleanType) {
				...
				res = await this.$api.info('jiankangfenlei', this.id);
		...
		methods: {
			...
			onPayTap(){
				...
				uni.setStorageSync('paytable','jiankangfenlei');
			...
			async onAcrossTap(tableName,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				uni.setStorageSync('crossTable','jiankangfenlei');
			...
			async init(type=1){
				...
				let res = await this.$api.info('jiankangfenlei', this.id);
				let reg=new RegExp('http://localhost:8080/springbootf73c0283/upload','g')//g代表全部
				this.detail = res.data;
				...
				if(type==2){
					...
					await this.$api.update('jiankangfenlei',this.detail)
				}
			},
